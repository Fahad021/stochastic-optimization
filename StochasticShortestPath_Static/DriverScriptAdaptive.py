...
if __name__ == "__main__":
    ...
    for theta in theta_list:
        ...
        print(f"************Starting iterations for theta {model.theta_step}")

        for _ in list(range(model.init_args['nIterations'])):
            model.obj = 0
            ...
            print(
                f"Finishing Theta {model.theta_step} and Iteration {model.n} with {step - 1} steps. Total cost: {model.obj} \n"
            )
            model.n+=1
    ...
    fig1.suptitle(
        f"Comparison of theta^step - stepsize type {M.init_args['stepsize_rule']} - origin {M.origin_node}, target {M.target_node}, dist {M.dist} "
    )
