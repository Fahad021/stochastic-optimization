...
def printTuple(a):
    return "".join(" {}: {:.2f}".format(f,getattr(a, f)) for f in a._fields) 

def printTupleValues(a):
    return "".join("{:.2f} ".format(getattr(a, f)) for f in a._fields)
...
#       
    ...
    if params['policy_central'] in ['regular', 'punishing']:
