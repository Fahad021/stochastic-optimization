...
if __name__ == "__main__":
	...
	print(
		f"Optimal order_quantity for price {price} and cost {cost} is {optimal_order_quantity}"
	)
	print(
	...
	plt.title(f"Analytical vs learned ordered quantity - (iteration {ite})")
	time = np.arange(0, len(order_quantity))
	plt.plot(time, time * 0 - np.log(cost/price) * 100, label = "Analytical solution")
	plt.plot(
		time, order_quantity, label=f"Kesten's Rule for theta_step {theta_step}"
	)
	plt.legend()
	plt.show()
