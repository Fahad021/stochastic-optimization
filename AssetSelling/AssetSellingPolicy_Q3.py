...
class AssetSellingPolicy():
    ...
    def track_policy(self, state, info_tuple):
        ...
        print(
            f'Theta {theta}, discount factor {info_tuple[1]},  Current price {state_price_d}, smoothed_price {smoothed_price}, d_smoothed_prince {smoothed_price}, and hold interval ({max(0, smoothed_price - theta)}, {smoothed_price + theta})'
        )
        return (
            {'sell': 1, 'hold': 0}
    ...
    def run_policy(self, param_list, policy_info, policy, time):
        ...
        print(f"obj={contribution}, state.resource={model_copy.state.resource}")
